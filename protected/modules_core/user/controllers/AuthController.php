...
class AuthController extends Controller
{
            ...
            if (isset($_POST['AccountRegisterForm'])) {
                ...
                if ($registerModel->validate()) {
                    ...
                    if (is_array($site)) {
                        ...
                        $siteModel = new UserSite();
                        $sites = [];
                        $hosts = [];
                        foreach ($site as $item) {
                            $item = $siteModel->fixURL($item, true);
                            $host = $siteModel->fixURL($item);
                            if ($validator->validateValue($item) !== false && !in_array($host, $hosts)) {
                                $sites[] = $item;
                                $hosts[] = $host;
                            }
                        }
                        ...
                        if(count($sites) > 0){
                            $userInvite->sites = json_encode($sites);
                        }
                    }
    ...
    public function actionCreateAccount()
    {
        ...
        if ($form->submitted('save') && $form->validate()) {
            ...
            if ($form['User']->model->save()) {
                ...
                // Save User Site
                if (!empty($userInvite->sites)) {
                    $sites = json_decode($userInvite->sites);
                    $guid = $form['User']->model->guid;
                    
                    if(count($sites) > 0){                        
                        foreach ($sites as $item) {
                            if (!empty($item)) {
                                $siteModel = new UserSite();
                                $siteModel->user_id = $guid;
                                $siteModel->url = $siteModel->title = $item;
                                $siteModel->setScenario('add');
                                $siteModel->save();
                            }
                        }
                    }
     ...
     */
    public function actionGenereteTestAccounts()
    {
        ...
        for ($i = $start; $i <= $stop; $i++) {
            ...
            $userPasswordModel = new UserPassword('newPassword');
            $userPasswordModel->newPassword = $userPasswordModel->newPasswordConfirm = $pass;
            $profileModel = $userModel->profile;
            $profileModel->scenario = 'register';
            ...
            if ($userModel->save()) {
                ...
                $profileModel->firstname = $name;
                $profileModel->lastname = $pass;
                
                $p = rand(0, 3);
                
                if($p >= 1){
                    $profileModel->mobile = '+' . rand(1, 9) . '(' . rand(100, 999) . ')' . rand(1000000, 9999999);
                }
                if($p >= 2){
                    $profileModel->phone_private = '+' . rand(1, 9) . '(' . rand(100, 999) . ')' . rand(1000000, 9999999);
                }
                if($p == 3){
                    $profileModel->phone_work = '+' . rand(1, 9) . '(' . rand(100, 999) . ')' . rand(1000000, 9999999);
                }
                ...
                $userPasswordModel->setPassword($userPasswordModel->newPassword);
                $userPasswordModel->save();
            }
        }
        
        die('created 10 test users');
    }
}
